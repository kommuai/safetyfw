...
void gpio_usart1_init(void) {
  // A2,A3: USART 1 for debugging
  set_gpio_alternate(GPIOB, 14, GPIO_AF7_USART2);
  set_gpio_alternate(GPIOB, 15, GPIO_AF7_USART2);
}
...
void common_init_gpio(void) {
  ...
  set_gpio_mode(GPIOD, 11, MODE_OUTPUT);
  set_gpio_output_type(GPIOD, 11, OUTPUT_TYPE_OPEN_DRAIN);

  set_gpio_pullup(GPIOD, 10, PULL_NONE);
  set_gpio_mode(GPIOD, 10, MODE_OUTPUT);
  set_gpio_output_type(GPIOD, 10, OUTPUT_TYPE_OPEN_DRAIN);

  set_gpio_pullup(GPIOD, 9, PULL_NONE);
  set_gpio_mode(GPIOD, 9, MODE_OUTPUT);
  set_gpio_output_type(GPIOD, 9, OUTPUT_TYPE_OPEN_DRAIN);

  /// E1: ~IG_SIM
  set_gpio_pullup(GPIOE, 1, PULL_NONE);
  set_gpio_mode(GPIOE, 1, MODE_OUTPUT);

  /// E2,E3,E4: MUX_SEL0, MUX_SEL1, MUX_EN
  set_gpio_pullup(GPIOE, 1, PULL_NONE);
  set_gpio_mode(GPIOE, 1, MODE_OUTPUT);
  ...
  set_gpio_mode(GPIOD, 15, MODE_INPUT);
